...
object ApplyEnsembleSpamClassifier {
  ...
  def main(argv: Array[String]) {
    ...
    val globalWeights_X = sc.broadcast(modelValues_X) // broadcast across all system nodes
    val globalWeights_Y = sc.broadcast(modelValues_Y)
    val globalWeights_Z = sc.broadcast(modelValues_B)

    // Scores a document based on its list of features [TAKEN FROM HANDOUT]
    def spamminess(features: Array[Int], weight: Map[Int, Double]) : Double = {
      var score = 0d
    ...
    val finalTestValues = testSamples.map(sample => {
      ...
      val spamValueX = spamminess(features, globalWeights_X)
      val spamValueY = spamminess(features, globalWeights_Y)
      val spamValueZ = spamminess(features, globalWeights_Z)
